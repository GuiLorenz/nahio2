...
const EditProfileInstituicaoScreen = ({ navigation }) => {
    const { userData, updateProfile } = useAuth();
    const [loading, setLoading] = useState(false);
    const [loadingCEP, setLoadingCEP] = useState(false);
    const [uploadingImage, setUploadingImage] = useState(false);

    // Estados do formulário (CNPJ/Telefone/CEP armazenam o valor LIMPO - somente números)
    const [nomeEscola, setNomeEscola] = useState('');
    const [cnpj, setCnpj] = useState(''); 
    const [telefone, setTelefone] = useState(''); 
    const [profileImage, setProfileImage] = useState('');

    // Endereço (CEP armazena valor puro)
    ...
    // Usamos useCallback para evitar re-criação desnecessária
    const loadUserData = useCallback(() => {
        if (userData?.profile) {
            const profile = userData.profile;
            setNomeEscola(profile.nomeEscola || '');
            
            // Carrega e LIMPA os valores brutos antes de salvar no estado
            setCnpj(profile.cnpj?.replace(/\D/g, '') || ''); 
    ...
    // --- Lógica de Inputs Auxiliares (Salvar valor puro no estado) ---
    // Funções que garantem que o estado sempre tenha o valor limpo (somente números)
    const handleCnpjChange = (text) => {
        setCnpj(text.replace(/\D/g, ''));
    };
        ...
        if (cepLimpo.length === 8) {
            ...
            try {
                ...
                if (resultado.success) {
                    ...
                    // Opcional: Limpar campos se a consulta falhar
    ...
    const validateForm = () => {
        if (!nomeEscola.trim()) {
            Alert.alert('Erro', 'Nome da escola é obrigatório.');
            return false;
        }
        // Validação usando o estado limpo (somente números)
        if (cnpj.length !== 14) { 
        ...
        <SafeAreaView style={globalStyles.safeArea}>
            <View style={styles.container}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>Editar Perfil da Instituição</Text>
                    <TouchableOpacity 
                ...
                >
                    ...
                    <View style={styles.profileImageSection}>
                        ...
                        <Text style={styles.profileImageLabel}>Toque para alterar logo</Text>
                    </View>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.inputContainer}>
                            ...
                            <TextInput
                                ...
                                maxLength={18} // Tamanho máximo da string mascarada
                            />
                        </View>
...
const styles = StyleSheet.create({
    ...
    editImageIconText: {
        fontSize: 16,
        color: colors.textPrimary, // Corrigido para ser visível
    },
    profileImageLabel: {
    ...
    row: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        // Adicionado margin vertical para a linha
        marginBottom: 15, 
    },
    flex1: {
        flex: 1,
...
export default EditProfileInstituicaoScreen;